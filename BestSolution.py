...
validation.to_csv('train_smooth_mean_target_real_features.csv')
...
predictors = validation.columns.difference(['name', 'track_num', 'is_listened', 'title'])
...
ans.to_csv('ans_smooth_mean_target_real_features.csv', index=False)

########################################################################################################
#Let's repeat with a new train set:)
validation = []
num_tracks = 6 
for i in range(num_tracks):
    print ('num_track:{}'.format(i))
...
validation.fillna(0, inplace=True)
validation.to_csv('train_smooth_mean_target_real_features1.csv')
print('train_shape:{}'.format(validation.shape))
print('train done')
...
ans.sort_values(by='sample_id', inplace=True)
ans['sample_id'] = ans.sample_id.astype(int)
ans.to_csv('ans_smooth_mean_target_real_features1.csv', index=False)
...
validation.fillna(0, inplace=True)
validation.to_csv('train_smooth_mean_target_real_features2.csv')
print('train_shape:{}'.format(validation.shape))
print('train done')
