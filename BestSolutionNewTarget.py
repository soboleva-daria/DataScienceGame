...
def build_model(train, test, target, X_test=None, num_tracks_skip=0):
    ...
    for i in range(num_tracks):
        ...
        print('1')
        v_new = smooth_mean_target(train, v_new, combinations(cat_features, 1), 'rate')#target)
        print('2')
        v_new = smooth_mean_target(train, v_new, combinations(cat_features, 2), 'rate')#target)
    ...
    predictors = X_train.columns.difference(['name', 'track_num', 'is_listened', 'title', 'rate'])
...
if __name__ == '__main__':
    ...
    new_target = pd.read_csv('new_targets.csv')
    train = pd.merge(train, new_target[['user_id', 'ts_listen', 'rate']],on=['user_id', 'ts_listen'], how='left')
    test = pd.merge(test, new_target[['user_id', 'ts_listen', 'rate']],on=['user_id', 'ts_listen'], how='left')
    ...
    print ('1')
    test = smooth_mean_target(train, test, combinations(cat_features , 1), 'rate')#target)
    print ('2')
    test = smooth_mean_target(train, test, combinations(cat_features , 2), 'rate')#target)
    ...
    ans = pd.read_csv('ans_smooth_meanNEW_target_real_features0.csv', index_col=0, squeeze=True)
    ans1 = pd.read_csv('ans_smooth_meanNEW_target_real_features1.csv', index_col=0, squeeze=True)
    ans2 = pd.read_csv('ans_smooth_meanNEW_target_real_features2.csv', index_col=0, squeeze=True)
